	...
	<div class="container" align="center">
		...
		<table class="table">
			...
			<c:if test="${not empty myCarpoolList }">
				<c:forEach var="cpList" items="${myCarpoolList }">
					<tr>
						<!-- 게시글 번호 순서 정렬 -->
						<c:if test="${cpList.CP_del == 'Y' }">
							<th colspan="5" class="col-md-2 text-center">삭제된 글 입니다</th>
						</c:if>
						<c:if test="${cpList.CP_del != 'Y' }">
							<td class="col-md-2 text-center">${num }
							<c:set var="num" value="${num -1 }"></c:set></td>
							<td class="col-md-2 text-center">
								<a href="cpView.do?CP_num=${cpList.CP_num }&pageNum=${pageNum}" class="btn btn-info btn-sm">
								이동</a></td>
							<td class="col-md-2 text-center">${cpList.CP_startDate }</td>
							<td class="col-md-2 text-center">${cpList.CP_startPoint }</td>
							<td class="col-md-2 text-center">${cpList.CP_endPoint }</td>
							
							<!-- 날짜 비교해서 후기 작성 버튼 활성화 만들것 -->
							<td class="col-md-2 text-center">
																										<!-- DMB_num에는 후기테이블에 작성자 컬럼에 들어가기 위해 카풀 게시글 등록자의 이름을 보냄 -->
								<button onclick="window.open('driverReservationList.do?CP_num=${cpList.CP_num}', '',
		...
		<div align="center">
			<ul class="pagination_bottom">
				...
				<c:if test="${pb.startPage > pb.pagePerBlock }">
					<li><a href="driveHistoryList.do?pageNum=1&search=${cpList.search }&keyword=${cpList.keyword }"
							class="page_num"><span class="glyphicon glyphicon-backward"></span></a></li>
					<li><a href="driveHistoryList.do?pageNum=${pb.startPage - 1 }&search=${cpList.search }&keyword=${cpList.keyword }"
						class="page_num"><span class="glyphicon glyphicon-triangle-left"></span></a></li>
				</c:if>
				<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }">
					<!-- 현재 머물고 있는 페이지가 몇 페이지인지 구별할 때 -->
					<c:if test="${pb.currentPage == i }">
						<li><a href="driveHistoryList.do?pageNum=${i }&search=${cpList.search }&keyword=${cpList.keyword }"
				...
				<c:if test="${pb.endPage < pb.totalPage }">
					<li><a href="driveHistoryList.do?pageNum=${pb.endPage + 1 }&search=${cpList.search }&keyword=${cpList.keyword }"
						class="page_num"><span class="glyphicon glyphicon-triangle-right"></span>
					</a></li>
					<li><a href="driveHistoryList.do?pageNum=${pb.totalPage }&search=${cpList.search }&keyword=${cpList.keyword }"
		...
		<form action="driveHistoryList.do">
			...
			<input type="text" name="keyword" value="${cpList.keyword }">	<!-- model에 추가해줘야한다 -->
