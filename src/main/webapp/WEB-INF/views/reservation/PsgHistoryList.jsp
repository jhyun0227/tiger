...
<body>
	<div class="container" align="center">
				...
				<c:forEach var="reservation" items="${myRvList }">
					<tr>
							...
							<td class="col-md-1 text-center">
								<a href="cpView.do?CP_num=${reservation.CP_num }&pageNum=${pageNum}" class="btn btn-info btn-sm">이동</a>
							</td>
							...
							<td class="col-md-2 text-center">
								<a onclick="window.open('profileView.do?MB_nickName=${reservation.MB_nickNameDv }', '',
									'width=500,height=1000,location=no,status=no,scrollbars=yes');">${reservation.MB_nickNameDv }</a>
							</td>
							...
							<td class="col-md-1 text-center">
								...
								<button onclick="window.open('psgReviewWriteForm.do?RSV_num=${reservation.RSV_num }&RV_writer=${reservation.MB_num }&RV_reader=${reservation.MB_numDv }&MB_nickName=${reservation.MB_nickNameDv }', '',
		...
		<div align="center">
			<ul class="pagination_bottom">
				...
				<c:if test="${pb.startPage > pb.pagePerBlock }">
					<li><a href="PsgHistoryList.do?pageNum=1&search=${reservation.search }&keyword=${reservation.keyword }"
							class="page_num"><span class="glyphicon glyphicon-backward"></span></a></li>
					<li><a href="PsgHistoryList.do?pageNum=${pb.startPage - 1 }&search=${reservation.search }&keyword=${reservation.keyword }"
						class="page_num"><span class="glyphicon glyphicon-triangle-left"></span></a></li>
				</c:if>
				<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }">
					<!-- 현재 머물고 있는 페이지가 몇 페이지인지 구별할 때 -->
					<c:if test="${pb.currentPage == i }">
						<li><a href="PsgHistoryList.do?pageNum=${i }&search=${reservation.search }&keyword=${reservation.keyword }"
				...
				<c:if test="${pb.endPage < pb.totalPage }">
					<li><a href="PsgHistoryList.do?pageNum=${pb.endPage + 1 }&search=${reservation.search }&keyword=${reservation.keyword }"
						class="page_num"><span class="glyphicon glyphicon-triangle-right"></span>
					</a></li>
					<li><a href="PsgHistoryList.do?pageNum=${pb.totalPage }&search=${reservation.search }&keyword=${reservation.keyword }"
		...
		<form action="PsgHistoryList.do">
			...
			<input type="text" name="keyword" value="${reservation.keyword }">	<!-- model에 추가해줘야한다 -->
