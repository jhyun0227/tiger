...
<body>
	<div class="container" align="center">
					...
					<tr>
						...
						<c:if test="${carpool.CP_del != 'Y' }">
							...
							<td class="col-md-2 text-center">
								<a href="cpView.do?CP_num=${carpool.CP_num }&pageNum=${pageNum}" class="btn btn-info btn-sm">
								이동</a></td>
							...
							<td class="col-md-2 text-center">
								...
								<button onclick="window.open('driverReservationList.do?CP_num=${carpool.CP_num}', '',
		...
		<div align="center">
			<ul class="pagination_bottom">
				...
				<c:if test="${pb.startPage > pb.pagePerBlock }">
					<li><a href="driverHistoryList.do?pageNum=1&search=${carpool.search }&keyword=${carpool.keyword }"
							class="page_num"><span class="glyphicon glyphicon-backward"></span></a></li>
					<li><a href="driverHistoryList.do?pageNum=${pb.startPage - 1 }&search=${carpool.search }&keyword=${carpool.keyword }"
						class="page_num"><span class="glyphicon glyphicon-triangle-left"></span></a></li>
				</c:if>
				<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }">
					<!-- 현재 머물고 있는 페이지가 몇 페이지인지 구별할 때 -->
					<c:if test="${pb.currentPage == i }">
						<li><a href="driverHistoryList.do?pageNum=${i }&search=${carpool.search }&keyword=${carpool.keyword }"
				...
				<c:if test="${pb.endPage < pb.totalPage }">
					<li><a href="driverHistoryList.do?pageNum=${pb.endPage + 1 }&search=${carpool.search }&keyword=${carpool.keyword }"
						class="page_num"><span class="glyphicon glyphicon-triangle-right"></span>
					</a></li>
					<li><a href="driverHistoryList.do?pageNum=${pb.totalPage }&search=${carpool.search }&keyword=${carpool.keyword }"
		...
		<form action="driverHistoryList.do">
			...
			<input type="text" name="keyword" value="${carpool.keyword }">	<!-- model에 추가해줘야한다 -->
